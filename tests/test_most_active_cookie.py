...
class TestCommandLineInterface:
    ...
    def test_parse_arguments_valid(self, monkeypatch):
        ...
        monkeypatch.setattr(
            sys, 'argv', 
            ['prog', '-f', 'cookie_log.csv', '-d', '2018-12-09']
        )
...
""")
        ...
        monkeypatch.setattr(
            sys, 'argv',
            ['prog', '-f', str(log_file), '-d', '2018-12-09']
        )
    ...
    def test_main_file_not_found(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            sys, 'argv',
            ['prog', '-f', '/nonexistent/file.csv', '-d', '2018-12-09']
        )
    ...
    def test_main_invalid_date(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            sys, 'argv',
            ['prog', '-f', str(log_file), '-d', 'invalid-date']
        )
